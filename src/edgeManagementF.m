function [nodes, edges, ages,fixed_nodes, error] = edgeManagementF(Input,nodes,edges,ages,fixed_nodes,error,distances,params)  %#codegen
...
if fixed_nodes(1,s1) == 1
    if norm(nodes(:,s1)-Input) > params(7)
        [nodes,edges,ages,fixed_nodes,error] = addNewNeuronF(Input,s1,nodes,edges,ages,fixed_nodes,error);
    else
        edges(s1,s2) = 1;
        edges(s2,s1) = 1;
        ages(s1,s2) = 0;
