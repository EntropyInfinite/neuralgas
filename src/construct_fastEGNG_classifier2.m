function [ nodes, edges, node_classes, failed ] = construct_fastEGNG_classifier2( Data, classLabels, NumOfEpochs, NumOfSamples, lambda)
...
for kk=1:NumOfEpochs
    ...
    if fixed_nodes(1,s1) == 1
        if norm(nodes(:,s1)-Input) > lambda
               ...
               classfreqs = [classfreqs zeros(NumOfClasses,1)];
        else
            edges(s1,s2) = 1;
            edges(s2,s1) = 1;
            ages(s1,s2) = 0;
            ages(s2,s1) = 0;
            indx = find(ClassIdents==classLabels(1,n));
            classfreqs(indx,s1) = classfreqs(indx,s1)+1;
        end
    else
    ...
    while NumOfNodes >= ii
        if any(edges(ii,:)) == 0
            ...
            classfreqs = [classfreqs(:,1:ii-1) classfreqs(:,ii+1:NumOfNodes);];
...
for n=1:NumOfNodes
    [vl maxindex] = max(classfreqs(:,n));
    if ClassIdents(1,maxindex)~=node_classes(1,n)
        if classfreqs(find(ClassIdents==node_classes(1,n)),n) < vl
            node_classes(1,n)=ClassIdents(1,maxindex);
        end
    end
end
